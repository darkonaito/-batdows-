...
if %errorlevel%==1 (
	...
	echo.
	set/p mapn=
	if exist !Alt.Prog_D!engine\maps\!mapn!.bat (
		set alrex=True
		goto menu
	) else (
		set "winPlacePos="
...
for /l %%i in (4,4,63) do (
 set pob=%%i
 set /a poc=%%i+1
 set /a pod=%%i+2
 set /a poe=%%i+3
for /f "tokens=1,2,3,4" %%g in ("!pob! !poc! !pod! !poe!") do echo บ!b%%g0!!b%%g1!!b%%g2!!b%%g3!!b%%g4!!b%%g5!!b%%g6!!b%%g7!!b%%g8!!b%%g9!!b%%h0!!b%%h1!!b%%h2!!b%%h3!!b%%h4!!b%%h5!!b%%h6!!b%%h7!!b%%h8!!b%%h9!!b%%i0!!b%%i1!!b%%i2!!b%%i3!!b%%i4!!b%%i5!!b%%i6!!b%%i7!!b%%i8!!b%%i9!!b%%j0!!b%%j1!!b%%j2!!b%%j3!!b%%j4!!b%%j5!!b%%j6!!b%%j7!!b%%j8!!b%%j9!บ %num%
)
...
echo m - Back To Menu        e - save ^& exit          x - Stuff
...
goto render
:save
echo set "winPlacePos=%winPlacePos%">"!Alt.Prog_D!engine\maps\"%mapn%.bat
echo set "deaPlacePos=%deaPlacePos%">>"!Alt.Prog_D!engine\maps\"%mapn%.bat
echo set "cols=%cols%">>"!Alt.Prog_D!engine\maps\"%mapn%.bat
echo color %cols%>>"!Alt.Prog_D!engine\maps\"%mapn%.bat
echo set "lvln=%mapn%">>"!Alt.Prog_D!engine\maps\"%mapn%.bat
echo set "nexLvlPos=%nexLvlPos%">>"!Alt.Prog_D!engine\maps\"%mapn%.bat
echo set "nexLvlN=%nexLvlN%">>"!Alt.Prog_D!engine\maps\"%mapn%.bat
echo set "loc_y=%loc_y%">>"!Alt.Prog_D!engine\maps\"%mapn%.bat
echo set "loc_x=%loc_x%">>"!Alt.Prog_D!engine\maps\"%mapn%.bat
echo set "ptr_l=%ptr_l%">>"!Alt.Prog_D!engine\maps\"%mapn%.bat
REM echo set "b1=!b%%i!">"engine\maps\"!mapn!.bat
for /l %%i in (1,1,639) do ( 
if "!b%%i!"=="%skin%" set "b%%i= "
echo set "b%%i=!b%%i!">>"!Alt.Prog_D!engine\maps\"%mapn%.bat
)
set "b%ptr_l%=%skin%"
goto render
:loadg
...
echo      Maps : [ 
echo.
dir/b !Alt.Prog_D!engine\maps\*
echo.
echo  ]
echo.
set/p mapn=
if not exist !Alt.Prog_D!engine\maps\!mapn!.bat goto menu
call "!Alt.Prog_D!engine\maps\"%mapn%.bat
set /a ptr_l=%loc_y%*40+%loc_x%
set "b%ptr_l%=%skin%"
goto render
:savexit
